...
bool SceneCloth::isValid(const int i, const int j) const {
    return i >= 0 && i < numParticlesX && j >= 0 && j < numParticlesY;
}


void SceneCloth::setProvotStretchSprings(const int i, const int j) {
    int iIndexes[2] = {0, 1}; // Only the directions going to the right to avoid repeated
    int jIndexes[2] = {1, 0};
    ...
    for(int l = 0; l < 2; ++l){
        ...
        int nj = j + jIndexes[l];
        if (isValid(ni, nj)) {
            Particle* p2 = system.getParticle(ni * numParticlesY + nj);
            double restLength = (p1->pos - p2->pos).norm();
            ForceSpring* f = new ForceSpring(p1, p2, restLength, widget->getStiffness(), widget->getDamping());
...
void SceneCloth::setProvotShearSprings(const int i, const int j){
    int iIndexes[2] = {-1, 1}; // Only the directions going to the right to avoid repeated
    int jIndexes[2] = {1, 1};
...
void SceneCloth::setProvotBendSprings(const int i, const int j){
    int iIndexes[2] = {0, 2}; // Only the directions going to the right to avoid repeated
    int jIndexes[2] = {2, 0};
...
void SceneCloth::computeProvot(const int i, const int j) {
    setProvotStretchSprings(i,j);
    setProvotShearSprings(i,j);
    setProvotBendSprings(i,j);
}
